...
class Reporter:
    def __init__(self, task_manager, bot_name, workspace_dir, gmail_user=None, gmail_app_password=None, report_to_email=None):
        self.tm = task_manager; self.bot_name = bot_name; self.workspace = workspace_dir
        self.gmail_user = gmail_user; self.gmail_app_password = gmail_app_password; self.report_to_email = report_to_email
        self.last_report_time = datetime.now(); self.last_output_time = datetime.now(); self.report_count = 0
    def generate_report(self):
        s = self.tm.get_status_summary(); now = datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        self.report_count += 1; return r
    def save_report(self):
        report = self.generate_report()
        with open(os.path.join(self.workspace, "status_report.md"), "w", encoding="utf-8") as f: f.write(report)
        return report
    def send_gmail_report(self, subject_prefix=""):
        ...
        report = self.generate_report(); msg = MIMEMultipart()
        msg['From'] = self.gmail_user; msg['To'] = self.report_to_email
        msg['Subject'] = f"{subject_prefix}[{self.bot_name}] Report #{self.report_count}"
        msg.attach(MIMEText(report, 'plain', 'utf-8'))
        try:
            with smtplib.SMTP_SSL('smtp.gmail.com', 465) as server:
                server.login(self.gmail_user, self.gmail_app_password); server.send_message(msg)
            return True
        except Exception as e: logger.error(f"Gmail failed: {e}"); return False
    ...
    def update_output_time(self): self.last_output_time = datetime.now()
