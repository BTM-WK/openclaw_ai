...
class AutonomousWorker:
    def __init__(self, task_manager, reporter, ai_router, bot_name, workspace_dir,
        ...
        self.task_interval = task_interval_seconds
    ...
    async def _execute_task(self, task):
        if not self._check_rate_limit():
            logger.warning("Rate limit, waiting 5min...")
    ...
    async def work_loop(self):
        ...
        self.tm.log_activity("Worker started")
        while self.is_running:
            try:
                ...
                if not task:
                    self.tm.log_activity("All tasks done. Waiting.")
                    await asyncio.sleep(60); continue
                logger.info(f"Exec: {task['id']} - {task['title']}")
                self.tm.log_activity(f"Start: {task['id']} - {task['title']}")
                resp = await self._execute_task(task)
                if resp is None:
                    self.tm.block_task(task["id"], "API failure")
                    await asyncio.sleep(60); continue
                ...
                if result:
                    if result.get("status") == "done":
                        ...
                        self.tm.complete_task(task["id"], of, summary)
                        self.tm.log_activity(f"Done: {task['id']} - {summary}")
                        ...
                        self.tm.block_task(task["id"], result.get("block_reason","Unknown"))
                else:
                    self.tm.complete_task(task["id"], "", "Auto-completed (unstructured)")
