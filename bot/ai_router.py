...
class AIRouter:
    ...
    def get_route_info(self, task_text):
        ...
        info = self.model_map.get(cat, self.model_map["general"]).copy()
        info["category"] = cat
        route = info.get("route", "anthropic")
        if route in ("prefer_openwebui", "openwebui") and self.check_openwebui():
            info["actual_route"] = "openwebui"
            info["actual_model"] = info.get("owui", info["model"])
        else:
            info["actual_route"] = "anthropic"
            info["actual_model"] = info["model"]
    ...
    def call(self, task_text, messages, system="", tools=None, max_tokens=4096, force_model=None, force_route=None):
        ...
        logger.info(f"Route: [{ri['category']}] -> {ri['actual_model']} via {ri['actual_route']}" +
                     (" (fallback)" if ri.get("fallback") else ""))
        if ri["actual_route"] == "openwebui":
            result = self.call_openwebui(ri["actual_model"], messages, system, max_tokens)
            if result.get("error"):
                result = self.call_anthropic(ri["model"], messages, system, tools, max_tokens)
    ...
    def get_status_text(self):
        ...
        for cat, info in self.model_map.items():
            ri = self.get_route_info(f"test {cat}")
            lines.append(f"  {cat}: {ri['actual_model']} via {ri['actual_route']}")
        return "\n".join(lines)
